...
type Declare struct{
	...
	PolicyID int `json:"id"`
 	// Statecode, 
 	// County, Eq_site_limit, Hu_site_limit, Fl_site_limit, Fr_site_limit, Tiv_2011, Tiv_2012, Eq_site_deductible, Hu_site_deductible, Fl_site_deductible, Fr_site_deductible, Point_latitude, Point_longitude, Line, Construction string
...
//===================================table row reading and write===============================
   ...
   fmt.Println("hi",id)
    ...
    rows, err2 := db.Query("SELECT policyID,point_granularity FROM detail_db WHERE policyid = $1",id)
	if err2 != nil {
	    log.Fatal(err2)
	}
    var d Declare
...
//========================================json=======================================
    ...
    d_json,err := json.Marshal(d_store[0])
